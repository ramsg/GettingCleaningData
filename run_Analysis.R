...
#Now extract only the mean and std columns from the merged data set
mean_std <- mergedData[,grepl("(mean\\(\\)|std\\(\\))", colnames(mergedData))]

# Add in the activity type and subject the newly created data table
mean_std[,67:68] <- mergedData[,562:563]
